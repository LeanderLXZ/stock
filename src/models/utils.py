...
def save_final_loss_log_to_csv(idx, log_path, loss_train_w_mean, loss_valid_w_mean, acc_train,
                               train_seed, cv_seed, n_valid, n_cv, parameters, profit=None):
    ...
    if not os.path.isfile(log_path + 'csv_log.csv'):
        ...
        with open(log_path + 'csv_log.csv', 'w') as f:
            if profit is None:
                ...
                header = ['id', 'time', 'profit', 'loss_train', 'loss_valid', 'train_accuracy',
                          'train_seed', 'cv_seed', 'n_valid', 'n_cv', 'parameters']
            writer = csv.writer(f)
    ...
    with open(log_path + 'csv_log.csv', 'a') as f:
        ...
        if profit is None:
            ...
            log = [idx, local_time, profit, loss_train_w_mean, loss_valid_w_mean, acc_train,
                   train_seed, cv_seed, n_valid, n_cv, str(parameters)]
        writer = csv.writer(f)
        writer.writerow(log)
...
def save_log_with_glv_to_csv(idx, log_path, loss_train_w_mean, loss_valid_w_mean, acc_train,
    ...
    if not os.path.isfile(log_path + 'csv_log.csv'):
        ...
        with open(log_path + 'csv_log.csv', 'w') as f:
            if profit is None:
                ...
                header = ['id', 'time', 'profit', 'loss_train', 'valid_loss', 'global_valid_loss', 'train_accuracy',
                          'global_valid_accuracy', 'train_seed', 'cv_seed', 'n_valid', 'n_cv', 'parameters']
            writer = csv.writer(f)
            writer.writerow(header)
    ...
    with open(log_path + 'csv_log.csv', 'a') as f:
        ...
        if profit is None:
            ...
            log = [idx, local_time, profit, loss_train_w_mean, loss_valid_w_mean, loss_global_valid_w_mean,
                   acc_train, acc_total_global_valid, train_seed, cv_seed, n_valid, n_cv, str(parameters)]
        writer = csv.writer(f)
        writer.writerow(log)


# Save Grid Search Log to csv File
def save_grid_search_log_to_csv(idx, log_path, loss_train_w_mean, loss_valid_w_mean, acc_train, train_seed, cv_seed,
                                n_valid, n_cv, parameters, param_name_list, param_value_list, profit=None):
...
def save_grid_search_log_with_glv_to_csv(idx, log_path, loss_train_w_mean, loss_valid_w_mean, acc_train,
                                         train_seed, loss_global_valid_w_mean, acc_total_global_valid, cv_seed,
                                         n_valid, n_cv, parameters, param_name_list, param_value_list, profit=None):
    ...
    if not os.path.isfile(log_path + 'csv_log.csv'):
        ...
        with open(log_path + 'csv_log.csv', 'w') as f:
            if profit is None:
                ...
                header = ['id', 'time', 'profit', 'loss_train', 'valid_loss', 'global_valid_loss',
                          'train_accuracy', 'global_valid_accuracy', *param_name_list, 'train_seed',
                          'cv_seed', 'n_valid', 'n_cv', 'parameters']
            writer = csv.writer(f)
            writer.writerow(header)
    ...
    with open(log_path + 'csv_log.csv', 'a') as f:
        ...
        if profit is None:
            ...
            log = [idx, local_time, profit, loss_train_w_mean, loss_valid_w_mean, loss_global_valid_w_mean, acc_train,
                   acc_total_global_valid, *param_value_list, train_seed, cv_seed, n_valid, n_cv, str(parameters)]
        writer = csv.writer(f)
        writer.writerow(log)
...
def print_positive_rate_test(era_sign_test=None):
    ...
    if era_sign_test is None:
        era_sign_test = load_pkl_to_data(cfg.prejudged_data_path + 'binary_era_sign_test.p')
